...
실행 중인 프로세스의 상태를 PCB에 적재하고, 실행하려는 프로세스의 정보를 PCB에서 읽어 레지스트리에 적재하는 것이다.

### PCB에 대해 설명해주세요

프로그램을 실행하기 위해 필요한 정보를 저장한다. 프로세스의 id, PC, 상태 비트, 레지스터 등을 가진다.
...
커널 영역을 공유하고 있으므로 하나의 스레드에 문제가 생기면 다른 스레드에도 문제가 생길 수 있다. 또한 자원을 공유하므로 임계구역에 대한 동기화가 필요하다.
