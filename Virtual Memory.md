...
**요구 페이징(paging)** 기법은 페이징 기법을 사용하는 가상 메모리 관리 기법이다. 가상 메모리는 고정된 크기로 나누어지는데 이 단위를 **페이지(page)**라고 하며, 실제 메모리에서 페이지와 동일한 크기의 **페이지 프레임(page frame)**이 페이지에 대응하게 된다.
...
**요구 세그먼테이션(segmentation) 기법**은 세그멘테이션 기법을 사용하는 가상 메모리 관리 기법이다. 프로세스는 의미 있는 단위로 분할되는데 이것을 **세그먼트(segment)**라고 한다.
...
## 요구 페이징과 요구 세그먼테이션의 비교

TODO

페이징 기법은 주소 공간을 고정된 크기의 페이지로 나누지만 세그멘테이션은 주소 공간을 가변 크기의 세그먼트로 나눈다. 페이징 기법에서 페이지 테이블의 엔트리는 페이지 하나를 가리키므로 테이블의 크기가 크지만 세그먼트 테이블의 엔트리는 세그먼트 하나를 가리키므로 테이블의 크기가 작다(??).
