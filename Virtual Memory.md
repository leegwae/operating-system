...
2. 가상 메모리의 크기에 비례하는 페이지 테이블의 크기(페이지 테이블 엔트리 개수와 페이지 개수가 같다)
...
#### 가상 주소의 P1, P2, Offset은 각각 몇 비트로 구성될까?

32비트 주소 체계이고, 페이지의 크기가 4KB(2^12바이트), 페이지 테이블 엔트리의 크기는 4바이트라고 하자.

- 바이트 오프셋의 크기는 페이지의 크기에 달려있다. 페이지의 크기는 4KB(2^12바이트)이므로 12비트면 오프셋을 나타낼 수 있다.
- P2 인덱스의 크기는 내부 페이지 테이블 하나의 엔트리의 개수에 달려있다. (내부 페이지 테이블 엔트리의 개수) = (페이지의 크기) / (페이지 테이블 엔트리의 크기)이므로 4KB(2^12바이트)/4바이트(2^2바이트)=2^10개이고, P2 인덱스의 크기는 10비트이다.
- P1 인덱스의 크기는 32비트에서 P2 인덱스와 바이트 오프셋을 제한만큼이다. 32비트- (10비트 + 12비트) = 10비트이다.

#### 페이지 테이블의 크기를 비교하라

32비트 주소 체계이고, 전체 주소 공간의 크기는 4GB(2^32바이트), 페이지의 크기는 4KB(2^12바이트), 페이지 테이블 엔트리의 크기는 4바이트라고 하자.

다단계 페이지 테이블을 사용하지 않는 경우 페이지 테이블의 크기는 아래와 같다.

```
(페이지 테이블의 크기) = (페이지 테이블 엔트리의 크기) * (페이지 테이블 엔트리의 개수)
```

페이지 테이블 엔트리의 개수는 곧 페이지의 개수이다.

```
(페이지 테이블 엔트리의 개수) = (페이지의 개수) = (전체 주소 공간의 크기) / (페이지의 크기)
```

그렇다면 페이지 테이블 엔트리의 개수는 4GB(2^32바이트) / 4KB(2^12바이트) = 2^20개이고, 페이지 테이블의 크기는 2^20 * 4바이트 = 4MB(2^22바이트)이다.

다단계 페이지 테이블을 사용하는 경우 외부 페이지 테이블의 크기는 아래와 같다.

```
(외부 페이지 테이블의 크기) = (외부 페이지 테이블 엔트리의 크기) * (외부 페이지 테이블 엔트리의 개수)
```

외부 페이지 테이블 엔트리의 크기는 외부 페이지 테이블 엔트리가 내부 페이지 테이블의 시작 주소를 저장하므로 4바이트이다. 외부 페이지 테이블 엔트리의 개수는 P1 인덱스의 크기에 달려있는데, P1 인덱스의 크기는 10비트이므로 외부 페이지 테이블 엔트리의 개수는 2^10개이다. 따라서 외부 페이지 테이블의 크기는 2^10 * 4바이트=4KB(2^12바이트)이다.

이젠 기본적으로는 4KB를 유지하고, 페이지 테이블 엔트리를 사용할 때마다 외부 페이지 테이블의 크기만큼 추가하면 된다.
...
프로세스의 수를 적절하게 조절하고 스레싱의 발생을 방지하는 방법에는 워킹셋 알고리즘과 PFF가 있다.
